...
import { Mic, Square, Loader2, Activity, Lightbulb } from 'lucide-react';
...
const AudioRecorder: React.FC<AudioRecorderProps> = ({ onRecordingComplete, isProcessing }) => {
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          if (data?.transcricao) onRecordingComplete(audioBlob, data.transcricao);
        } catch (error) {
          console.error(error);
      ...
      toast({
        ...
        className: "bg-blue-50 border-blue-200 text-blue-800", // Estilo personalizado opcional
      });

    } catch (error) {
      toast({ title: 'Erro', description: 'Permita o uso do microfone.', variant: 'destructive' });
    }
  ...
  return (
    <div className="flex flex-col items-center justify-center py-10 w-full">
      ...
      {/* 1. Área do Botão */}
      <div className="relative mb-6"> {/* Margem inferior para separar do texto */}
        <Button
      ...
      {/* Removi o 'absolute' e coloquei uma altura fixa (h-16) para evitar pulos na tela */}
      <div className="w-64 text-center h-16 flex flex-col items-center justify-start gap-2">
        <p className={`text-lg font-medium transition-colors duration-300 ${isRecording ? 'text-destructive' : 'text-muted-foreground'}`}>
          {isProcessing 
      ...
      {!isRecording && !isProcessing && (
        <div className="mt-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div className="bg-blue-50/80 border border-blue-100 p-4 rounded-xl max-w-md flex items-start gap-3 text-left shadow-sm">
            ...
            <div>
              <p className="text-sm font-bold text-blue-900 mb-1">Dica para melhor resultado:</p>
              <p className="text-sm text-blue-700 leading-relaxed">
                Para uma análise precisa, relate com detalhes o que o paciente está sentindo, sintomas e duração.
              </p>
            </div>
          </div>
        </div>
