...
import { AlertCircle, FileText, Pill, Stethoscope } from "lucide-react";
...
const AnalysisReport = ({ analise }: { analise: Analise }) => {
	return (
		<div
			...
			className="w-full max-w-4xl mx-auto mt-8 animate-medical-reveal"
		>
			<Card className="border-none shadow-2xl bg-white/80 backdrop-blur-sm overflow-hidden">
				...
				<div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 text-white">
					...
					<p className="text-blue-100 opacity-90 pl-11">
						Diagnóstico gerado em tempo real
					</p>
				</div>
						...
						<div className="space-y-4">
							...
							<ul className="space-y-3">
								{analise.medicamentosSugeridos.map((item, i) => (
									<li
										key={i}
										className="flex items-start gap-3 bg-purple-50/50 p-3 rounded-lg hover:bg-purple-50 transition-colors"
									>
						...
						<div className="space-y-4">
							...
							<ul className="space-y-3">
								{analise.examesRecomendados.map((item, i) => (
									<li
										key={i}
										className="flex items-start gap-3 bg-emerald-50/50 p-3 rounded-lg hover:bg-emerald-50 transition-colors"
									>
