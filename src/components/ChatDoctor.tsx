...
import { Send, Bot, User, MessageSquarePlus, Loader2, Download } from 'lucide-react';
...
const gerarAtestadoPDF = (data: AtestadoData) => {
  ...
  // Usamos 'splitTextToSize' para quebrar a linha automaticamente
  ...
  // Localização fictícia, ajuste se necessário
  doc.text(`Duque de Caxias, ${dataAtual}.`, 105, 100, { align: 'center' }); 
  
  doc.text("___________________________________", 105, 120, { align: 'center' });
...
const ChatDoctor: React.FC<ChatProps> = ({ contextoAnalise, onOpen }) => {
  ...
  const handleSend = async () => {
    ...
    // Avisa a página 'Consulta' que o chat iniciou (na primeira msg)
    ...
    try {
      ...
      // --- MÁGICA DA DETECÇÃO ---
      try {
        const parsedJson = JSON.parse(data.resposta);
        // Se for um JSON e tiver o tipo ATESTADO
        if (parsedJson && parsedJson.tipo === 'ATESTADO') {
          iaMsgContent = "Gerei o atestado solicitado. Clique no botão abaixo para fazer o download.";
          atestadoData = parsedJson;
        }
      } catch (e) {
        // Não era um JSON, só texto normal. Segue o jogo.
      }
      ...
      setMessages(prev => [...prev, { 
        ...
        atestadoData: atestadoData // Anexa os dados do PDF à mensagem
      }]);
  ...
  // Esta função chama o onOpen na primeira vez que o usuário foca o input
  const handleFocus = () => {
    if (!hasInteracted && onOpen) {
      onOpen();
      setHasInteracted(true);
        ...
        <div className="h-[300px] overflow-y-auto p-4 space-y-4 bg-slate-50/50">
          {messages.length === 0 && (
            <div className="text-center text-muted-foreground mt-10 opacity-60">
              ...
              <p className="text-sm italic">"Qual a posologia infantil para o medicamento sugerido?"</p>
            </div>
          )}
          ...
          {messages.map((msg, idx) => (
            ...
            >
              <div className={`flex max-w-[80%] gap-2 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${msg.role === 'user' ? 'bg-primary text-white' : 'bg-emerald-600 text-white'}`}>
