              ...
              `}
							>
								...
								{isCompleted && index < currentStep ? (
									<CheckCircle className="h-7 w-7 text-white relative z-10" />
								) : isCompleted && index === currentStep ? (
									<CheckCircle className="h-7 w-7 text-white relative z-10 animate-bounce" />
								) : (
									<Icon
						...
						{/* Linha de conexão modernizada */}
						{index < steps.length - 1 && (
							<div
								className={`
                absolute top-7 left-[calc(50%_+_4rem)] w-[calc(100%_-_3rem)] h-1 -z-10
                hidden md:block rounded-full
                transition-all duration-500 overflow-hidden
                ${isCompleted ? "bg-gradient-to-r from-green-500 to-emerald-500" : "bg-slate-800/50"}
              `}
							>
								{isCompleted && (
		...
		<div className="min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-slate-900 relative overflow-x-hidden">
			...
			<div className="relative z-10 min-h-screen flex flex-col">
				...
				<header className="sticky top-0 z-50 bg-slate-900/60 backdrop-blur-2xl border-b border-white/10 shadow-lg shadow-black/20">
					<div className="container mx-auto px-4 h-20 flex justify-between items-center relative overflow-x-hidden">
						{/* Bloco Título (Animado) */}
						<div
							className={`
              ...
              `}
						>
							<div className="relative group">
								...
								<div className="relative bg-gradient-to-br from-blue-500 to-purple-600 w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center shadow-xl">
									<HeartHandshake className="h-6 w-6 md:h-7 md:w-7 text-white" />
								</div>
							</div>
							<div>
								<h1 className="text-xl md:text-2xl font-bold bg-gradient-to-r from-blue-200 via-blue-100 to-purple-200 bg-clip-text text-transparent">
									Médico Copilot
								</h1>
								<p className="text-xs md:text-sm text-blue-200/60 flex items-center gap-2">
						...
						{/* Bloco Histórico (Mobile, Animado) */}
						<div
							className={`
              ...
              `}
                  ...
                  transition-all duration-300 ease-in-out rounded-2xl px-8 py-6
                "
							>
								<History className="h-10 w-10 mr-4 transition-transform duration-300 group-hover:-rotate-12" />
                  ...
                  transition-all duration-300 ease-in-out rounded-xl px-6 py-5
                "
							>
								<History className="h-5 w-5 mr-2 transition-transform duration-300 group-hover:rotate-12" />
						...
						<div className="md:hidden z-10 ml-auto">
							<Button
								...
								className="text-white hover:bg-white/10 transition-all duration-300 transform hover:scale-110 rounded-xl w-12 h-12"
							>
								{isMenuOpen ? (
									<X className="h-7 w-7" />
								) : (
				...
				<main className="container mx-auto px-4 py-8 max-w-7xl flex-1">
					{/* Step Indicator */}
					<Card className="mb-8 border-0 shadow-2xl bg-slate-900/40 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden">
					...
					{/* Barra de Progresso */}
					{isProcessing && (
						<Card className="mb-6 border-0 bg-gradient-to-r from-slate-800/80 to-slate-900/80 backdrop-blur-xl shadow-xl rounded-2xl overflow-hidden animate-in fade-in slide-in-from-top-4 duration-500">
							...
							<CardContent className="pt-6 pb-6">
								<div className="flex items-center justify-between mb-3">
									...
									<span className="text-sm text-blue-300 font-mono bg-blue-950/50 px-3 py-1 rounded-full">
										{progress}%
									</span>
								</div>
								<Progress value={progress} className="h-2.5 bg-slate-800/50" />
								<p className="text-xs text-blue-200/60 mt-3 flex items-center gap-2">
					...
					<div className="grid lg:grid-cols-3 gap-6">
						...
						<div className="lg:col-span-2 space-y-6">
							...
							<Card className="shadow-2xl border-0 bg-slate-900/40 backdrop-blur-xl overflow-hidden rounded-2xl border border-white/10 hover:border-blue-500/30 transition-all duration-300 group">
								...
								<CardHeader className="bg-gradient-to-r from-blue-900/20 to-purple-900/20">
									<CardTitle className="flex items-center justify-between">
										<span className="flex items-center gap-3 text-blue-100">
							...
							{/* Card de Transcrição */}
							{transcricao && (
								<Card className="shadow-2xl border-0 bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-xl animate-in fade-in slide-in-from-bottom-4 duration-700 rounded-2xl border border-white/10 overflow-hidden">
									<div className="h-1 bg-gradient-to-r from-green-500 to-emerald-500"></div>
									<CardHeader className="bg-gradient-to-r from-green-900/20 to-emerald-900/20">
									...
									<CardContent className="p-6">
										<div className="bg-slate-950/50 p-6 rounded-xl border border-green-500/10 backdrop-blur-sm">
						...
						<div className="space-y-6">
								...
								<Card className="shadow-2xl border-0 bg-gradient-to-br from-slate-900/50 to-blue-900/30 backdrop-blur-xl rounded-2xl border border-white/10 overflow-hidden">
									...
									<CardContent className="space-y-3 p-6">
										<Button
											...
											className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white shadow-lg shadow-blue-500/20 hover:shadow-xl hover:scale-105 transition-all duration-300 rounded-xl h-12 font-medium"
										>
										...
										<Button
											variant="outline"
											className="w-full bg-slate-800/50 border-slate-700 text-slate-200 hover:bg-slate-700 hover:border-blue-500/50 transition-all duration-300 rounded-xl h-11"
							...
							{/* Dicas de Uso */}
							<Card className="shadow-2xl border-0 bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-xl rounded-2xl border border-white/10 overflow-hidden">
								...
								<CardHeader className="bg-gradient-to-r from-emerald-900/20 to-green-900/20">
									<CardTitle className="text-lg flex items-center gap-2 text-emerald-100">
								...
								<CardContent className="p-6">
									<ul className="space-y-3">
										<li className="flex items-start gap-3 group">
											<div className="h-6 w-6 rounded-lg bg-green-500/10 flex items-center justify-center mt-0.5 group-hover:bg-green-500/20 transition-colors">
